...
const DEFAULT_ACTOR_ID = process.env.APIFY_JOBS_ACTOR_ID || 'valig~linkedin-jobs-scraper';
// Plafond de résultats par mot-clé pour maîtriser le coût du run.
const MAX_ITEMS = Number(process.env.APIFY_JOBS_MAX_ITEMS || '25');
...
export const apifyScraper: Scraper = {
  ...
  async fetch(keywords: string[], opts: { location?: string; credentials: Record<string, string> }): Promise<ScraperResult> {
    ...
    const budgetMs = Number(process.env.APIFY_JOBS_BUDGET_MS || DEFAULT_BUDGET_MS);
